...
export default function MyAppliedList() {
  ...
  const fetchPosts = async () => {
    try {
      const data = await apiService.getMyAppliedJoins();
      setPosts(data);
    } catch (e) {
      console.log('참가 조인글 로딩 에러:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>신청했거나 참가 중인 스크린 골프 조인이 없습니다.</Text>
            <Text style={styles.emptySubText}>조인 모집 목록에서 마음에 드는 조인에 참가해보세요!</Text>
          </View>
        }
      />
      {selectedPostId && (
